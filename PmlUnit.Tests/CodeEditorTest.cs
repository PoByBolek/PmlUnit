...
namespace PmlUnit.Tests
{
    ...
    public class CodeEditorDescriptorTest
    {
        ...
        [TestCase(CodeEditorKind.Atom, @"C:\Program Files\Atom\atom.exe", "random stuff", typeof(AtomCodeEditor))]
        [TestCase(CodeEditorKind.Atom, @"C:\Users\Current\AppData\Local\Atom\atom.exe", "other stuff", typeof(AtomCodeEditor))]
        [TestCase(CodeEditorKind.NotepadPlusPlus, @"C:\Program Files\Notepad++\notepad++.exe", "foobar", typeof(NotepadPlusPlusCodeEditor))]
        ...
        [TestCase(CodeEditorKind.SublimeText, @"C:\Program Files\Sublime Text 3\subl.exe", "empty", typeof(SublimeTextCodeEditor))]
        [TestCase(CodeEditorKind.UltraEdit, @"C:\Program Files\UltraEdit\uedit64.exe", "--test", typeof(UltraEditCodeEditor))]
        [TestCase(CodeEditorKind.UltraEdit, @"C:\Program Files (x86)\UltraEdit\uedit32.exe", "--clear", typeof(UltraEditCodeEditor))]
        [TestCase(CodeEditorKind.VisualStudioCode, @"C:\Users\Current\AppData\Local\Programs\Microsoft VS Code\Code.exe", "stuff", typeof(VisualStudioCodeCodeEditor))]
        [TestCase(CodeEditorKind.VisualStudioCode, @"C:\Program Files\Microsoft VS Code\Code.exe", "random", typeof(VisualStudioCodeCodeEditor))]
        [TestCase(CodeEditorKind.Other, @"C:\Program Files\Editor\Editor.exe", "--be-awesome", typeof(OtherCodeEditor))]
        [TestCase(CodeEditorKind.Other, @"C:\Program Files\Something Else\foo.exe", "--bar", typeof(OtherCodeEditor))]
        public void CreatesCodeEditorInstance(int kind, string fileName, string fixedArguments, Type expectedType)
        {
            ...
            Assert.That(fixedArgumentsField.GetValue(editor), Is.EqualTo(fixedArguments));
        }
    }
    ...
    public class CodeEditorTest
    {
        ...
        [TestCase("fixed arguments should\"be copied untouched \"dynamic arguments should come after the fixed ones\"", "fixed arguments should\"be copied untouched", "dynamic arguments should come after the fixed ones")]
        ...
        public void BuildsProcessStartInfoCorrectly(Type editorType, string fileName, int lineNumber, string expectedArguments)
        {
            var constructor = editorType.GetConstructor(new Type[] { typeof(string), typeof(string) });
            var editor = constructor.Invoke(new object[] { @"C:\Program Files\Editor\Editor.exe", "" });
